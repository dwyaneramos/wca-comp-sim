...
import { PLAYER_ID, DNF, MO3_EVENTS, BIG_EVENTS, MOBILE_BREAKPOINT } from "../../utils/constants.js"
...
const EditTimePopup = ({ cuber, idx, onClick }) => {
  ...
  return (
    <div className="bg-white border-2 border-gray-200 flex gap-2 justify-center items-center flex-col w-100 h-50 fixed right-0 left-0 mx-auto top-83 sm:top-100 z-100 sm:top-0 sm:bottom-0 my-auto">
      <h1>Edit Time</h1>
...
const MobileTimesDisplay = ({ cuber, solveNum, records, recordColorLookup, setShowPopup, setShowTimesOnMobile, showTimesOnMobile }) => {
  ...
  return (
    <div className="fixed text-center flex flex-col px-3 pt-5 bg-white border-3 border-gray-200 top-85 right-0 left-0 mx-auto h-100 w-[95vw]">
      ...
      <button type="" onClick={() => { setShowTimesOnMobile(null); setShowPopup({ cuber: null, solveIdx: null }) }} className="absolute right-2 top-2 cursor-pointer hover:bg-gray-100 p-3 text-2xl rounded-md border-2 border-gray-200">
        <RxCross1 size={20} />
      </button>
